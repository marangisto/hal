...
template<> void handler<interrupt::TAMPER>() __attribute__ ((weak, alias("_Z17__default_handlerv")));
...
void (*vectors[])(void) __attribute__ ((section(".vectors"))) =
    ...
    , handler<interrupt::TAMPER>                    // 2: Tamper interrupt
    , handler<interrupt::RTC>                       // 3: RTC global interrupt
    ...
    , handler<interrupt::USB_HP_CAN_TX>             // 19: USB High Priority or CAN TX interrupts
    , handler<interrupt::USB_LP_CAN_RX0>            // 20: USB Low Priority or CAN RX0 interrupts
    , handler<interrupt::CAN_RX1>                   // 21: CAN RX1 interrupt
    , handler<interrupt::CAN_SCE>                   // 22: CAN SCE interrupt
    ...
    , handler<interrupt::TIM1_TRG_COM>              // 26: TIM1 Trigger and Commutation interrupts
    ...
    , handler<interrupt::TIM6>                      // 54: TIM6 global interrupt
